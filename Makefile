...
ifeq ($(PLATFORM),device)
  ...
  ifeq ($(HOST),windows)
    ...
    CXX = g++
    GDB = gdb --args
    SIMULATOR = numworks-epsilon # That is if you use numworks-epsilon AUR package
    #EXE = bin # Comment out the line above to use the prebuilt simulator and uncomment this one
  else
    CXX = clang++
    GDB = lldb --
    EXE = app
...
else ifeq ($(OS),macos) # Comment out this line to use the prebuilt simulator
...
$(BUILD_DIR)/icon.o: src/eadk/icon.png
	@echo "ICON    $<"
	$(Q) $(NWLINK) png-icon-o $< $@
