...
with open("textures.h", "w") as output:
    ...
    for i in range(len(palette)):
        if i != len(palette) - 1:
            ...
            tmpString += palette[i]
    output.write("int defltPallette[16] = \n    {" + tmpString + "};\n\n")
    ...
    output.write(f"int mainSprtSheet[(128 / {spriteSize}) * (64 / {spriteSize})][{spriteSize}][{spriteSize}] = \n" + "    {" + tmpString + "};\n\n")
    ...
    tmpString = prettyDataFormat(tmpSprData)
    output.write(f"int fontSprtSheet[(128 / {spriteSize}) * (80 / {spriteSize})][{spriteSize}][{spriteSize}] = \n" + "    {" + tmpString + "};")